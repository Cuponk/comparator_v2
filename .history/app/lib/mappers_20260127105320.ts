import { fetchJudge, fetchOhtani } from "./mlb-api";
...
export async function getMappedJudge() {
    const rawJudge = await fetchJudge();
    return mapPlayerData(rawJudge);
}

export async function getMappedOhtani() {
    const rawOhtani = await fetchOhtani();
    return mapPlayerData(rawOhtani);
}
